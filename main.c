...
int ETUDIANTS(IdEtudiant id, Etudiant nouv_etudiant) {
...
int NOTE(IdEtudiant id, Etudiant nouv_etudiant) {
	int id_etudiant;
	// Vérifie si l'ID est valide.
	scanf_s("%d %d %f", &id_etudiant, &nouv_etudiant.UE, &id.liste_etudiants->note);
	for (int i = 0; i < id.nombre_etudiant; ++i) {
...
;
	...
	do {
		...
		if (strcmp(commande, "ETUDIANTS") == 0) {
			ETUDIANTS(id, nouv_etudiant);
		}
		else if (strcmp(commande, "CURSUS") == 0) {
			//CURSUS(id, nouv_etudiant);
		}
